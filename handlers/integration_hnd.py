...
def integrate_parser():
    p = argparse.ArgumentParser(
        ...
        epilog="Пример: python mathtool.py integral --func root --from 1 --to 7 --step 9",
        allow_abbrev=False,
    )
    ...
    p.add_argument('--step', default=1, type=int, help='шаг интегрирования')
...
def handle_integrate(args):
    ...
    step = args.step
    return validator(func, start, end, step)



def validator(func, start, end, step):
    if start >= end:
    ...
    f, low, high, strict = FORMULS[func]
    ...
    return {"func": f, "start": start, "end": end, "step": step}
